...
- **Invalid LocalBusiness structured data fixed — 26 pages (2026-09-22 — DONE)** — Semrush flagged 11 pages, "Local Business / 1 field". Root cause: nested `provider` / `worksFor` / `mainEntity` stubs restated `"@type": "ProfessionalService"` alongside `"@id": ".../#organization"`. In JSON-LD an `@id` **alone** is a pointer; adding `@type` turns it into a second, standalone LocalBusiness declaration — one with no `address` (a required field), and an `@id` collision with the complete root node. A deep scan of all 43 built pages found 26 affected, not 11. Fixes: (1) all stubs collapsed to pure refs `{ "@id": ".../#organization" }`; (2) 12 location pages changed `ProfessionalService` -> `Service` + `serviceType` + `provider` ref (dropped `telephone`/`hasCredential`/`parentOrganization`, which are Organization properties) — `Service` needs no address and avoids implying 12 physical offices for a service-area business; (3) deleted the duplicate `#localbusiness` node on `/about` (worse copy of the root node: partial address, `closes: "23:59"`); (4) `/reviews` node stripped of `@type` so it merges `aggregateRating` into the org entity instead of redeclaring it. Verified on the production build: **0 of 43 pages** have an invalid LocalBusiness-family node. **Open item:** `/reviews` writes a live Google `aggregateRating` for `#organization` while the root layout hardcodes 4.9/67 — identical today, but they will drift. Root should read the live value or the page node should be the only source. **Expectations:** this is hygiene — it clears an Error-severity item and will lift the Semrush health score, but LocalBusiness markup produces no rich result in classic organic search, so no direct ranking gain. Real levers remain `/infidelity` + `/child-custody` rewrites and GBP access.
